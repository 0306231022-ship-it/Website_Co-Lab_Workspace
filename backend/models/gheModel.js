...
export default class GheModel {
  ...
  static async getIDkhongian(idkhongigan) {
    try {
      ...
      throw new Error(
        "Không thể kết nối đến cơ sở dữ liệu để lấy danh sách ghế!",
      );
    }
  }
  ...
  static async getById(id) {
    try {
      ...
      console.error(
        ` Lỗi Database trong GheModel.getById (${id}):`,
        error.message,
      );
  ...
  static async create(data) {
    try {
      const [create] = await execute(
        `INSERT INTO ghe (TEN_GHE,TOA_X,TOA_Y,TRANG_THAI,ID_KHONG_GIAN,ID_DANH_MUC) 
            VALUES(?,?,?,1,?,?)`,
        [
          data.TEN_GHE,
          data.TOA_X,
  ...
  static async update(id, data) {
    try {
      ...
      console.error(
        `❌ Lỗi Database trong GheModel.update (${id}):`,
        error.message,
      );
