...
export default class giaModel {
    ...
    static async getAll(offset , limit) {
        try {
            const [rows] = await execute(
                "SELECT * FROM banggia LIMIT ? OFFSET ?", 
                [limit, offset]
            );
            const [totalRows] = await execute("SELECT COUNT(*) as total FROM banggia");
    ...
    // 2. Lấy giá  theo ID (Không cần LIMIT/OFFSET vì ID là duy nhất)
     static async getById(id) {
        console.log(id)
        try {
            const [rows] = await execute("SELECT * FROM banggia WHERE ID_GIA = ?", [id]);
            return rows[0];
        } catch (error) {
            console.error(` Lỗi Database trong getById (${id}):`, error.message);
            throw new Error("Không thể kết nối đến cơ sở dữ liệu để lấy chi tiết gia!");
        }
    }
