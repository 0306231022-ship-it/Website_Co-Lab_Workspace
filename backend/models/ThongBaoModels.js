...
export default class thongBaoModel {
  ...
  static async getAll(offset, limit) {
    try {
      ...
      console.error(
        "❌ Lỗi Database trong thongBaoModel.getAll:",
        error.message,
      );
  ...
  static async getByUserId(idnd, offset, limit) {
    try {
      ...
      const [totalRows] = await execute(
        "SELECT COUNT(*) as total FROM thongbao WHERE IDND = ?",
        [idnd],
      );
      const total = totalRows[0]?.total || 0;
      ...
      console.error(
        `❌ Lỗi Database trong thongBaoModel.getByUserId (${idnd}):`,
        error.message,
      );
  ...
  static async create(tieuDe, noiDung, loaiThongBao, idnd) {
    try {
      ...
      console.error(
        "❌ Lỗi Database trong thongBaoModel.create:",
        error.message,
      );
  ...
  static async deleteById(idThongBao) {
    try {
      const [result] = await execute(
        "DELETE FROM thongbao WHERE ID_THONGBAO = ?",
        [idThongBao],
      );
      return result.affectedRows > 0;
    } catch (error) {
      console.error(
        `❌ Lỗi Database trong thongBaoModel.deleteById (${idThongBao}):`,
        error.message,
      );
  ...
  static async deleteAllByUserId(idnd) {
    try {
      const [result] = await execute("DELETE FROM thongbao WHERE IDND = ?", [
        idnd,
      ]);
      return result.affectedRows > 0;
    } catch (error) {
      console.error(
        `❌ Lỗi Database trong thongBaoModel.deleteAllByUserId (${idnd}):`,
        error.message,
      );
  ...
  static async testId(idThongBao) {
    try {
      const [rows] = await execute(
        "SELECT ID_THONGBAO FROM thongbao WHERE ID_THONGBAO = ?",
        [idThongBao],
      );
