...
export default class hoadonController {
   ...
   static async getHoaDonById(req, res) {
        try {
            ...
            await Promise.all([
                query('ID_LICH_DAT') 
                    .notEmpty().withMessage('ID hóa đơn không được bỏ trống!')
                    .isInt({ min: 1 }).withMessage('ID hóa đơn phải là số nguyên dương!')
                    .custom(async (value) => {
                        const check = await DatLichModel.kiemtraid(value);
                        if (!check) throw new Error('Mã hóa đơn này không tồn tại trên hệ thống!');
                        return true;
                    })
                    .run(req)
            ]);
            ...
            if (!item) {
                return res.status(404).json({ success: false, message: "Không tìm thấy thông tin hóa đơn!" });
            }
    ...
    static async createHoaDon(req, res) {
        try {
            ...
            // Validate toàn bộ các trường của bảng hóa đơn bằng Promise.all giống hệt mẫu ghế
            await Promise.all([
                body('GIA_TIEN')
                    .notEmpty().withMessage('Giá tiền không được bỏ trống')
                    .isFloat({ min: 0 }).withMessage('Giá tiền phải là số thập phân hoặc số nguyên dương lớn hơn hoặc bằng 0')
                    .run(req),
           
                body('ID_LICHDAT')
                    .notEmpty().withMessage('id lịch đặt không được bỏ trống')
                    .isInt().withMessage('id lịch đặt phải là số nguyên')
                    .custom(async (value, { req }) => {
                        // Kiểm tra ID_LICHDAT có tồn tại thực tế hay không (Giống hệt cách check ID_KHONG_GIAN của em)
            ...
            res.status(200).json({ success: true, message: "Thêm hóa đơn thành công!" });
        } catch (error) {
