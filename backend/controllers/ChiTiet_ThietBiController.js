...
export default class ChiTietThietBiController {
    ...
    try {
      await Promise.all([
        body("ID_KHONG_GIAN")
          ...
          .isInt({ min: 1 })
          .withMessage("ID không gian không được nhỏ hơn 1!")
          .custom(async (value) => {
        ...
        body("ID_THIET_BI")
          .notEmpty()
          .withMessage("ID thiết bị là thông tin bắt buộc")
          .isInt()
          .withMessage("Giá trị nhập vào phải là một số nguyên!")
          .isInt({ min: 1 })
          .withMessage("ID thiết bị không được nhỏ hơn 1!")
          .custom(async (value) => {
  ...
  static async DanhSach_ThietBi(req, res) {
    ...
    try {
      ...
      const DanhSach = await ChiTietThietBiModel.DanhSachThietBi_Khonggian(
        IDKG,
        limit,
        offset,
      );
