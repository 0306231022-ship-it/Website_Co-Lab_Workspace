  ...
  static async getGheById(req, res) {
    try {
      ...
      if (!item) {
        return res
          .status(404)
          .json({ success: false, message: "Không tìm thấy thông tin ghế!" });
      }
      res.status(200).json({
        success: true,
        dulieu: {
    ...
    try {
      ...
      await Promise.all([
        body("TEN_GHE")
          ...
          .isLength({ max: 255 })
          .withMessage("Tên ghế tối đa 255 ký tự")
          .run(req),
        body("TOA_X")
          .notEmpty()
          .withMessage("tọa độ X không được bỏ trống")
          .isInt()
          .withMessage("tọa độ X phải là số số thập phân hoặc số nguyên")
          .run(req),
        body("TOA_Y")
          .notEmpty()
          .withMessage("tọa độ Y không được bỏ trống")
          .isInt()
          .withMessage("tọa độ Y phải là số số thập phân hoặc số nguyên")
          .run(req),
        body("ID_KHONG_GIAN")
          .notEmpty()
          .withMessage("id không gian không được bỏ trống")
          .isInt()
          .withMessage("id không gian phải là số nguyên")
        ...
        body("ID_DANH_MUC")
          .notEmpty()
          .withMessage("id danh mục không được bỏ trống")
          .isInt()
      ...
      res
        .status(200)
        .json({ success: true, message: "Thêm ghế thành công!", id: insertId });
    } catch (error) {
      res.status(500).json({ success: false, message: error.message });
    }
  }
  ...
  static async updateGhe(req, res) {
    try {
      await Promise.all([
        ...
        body("TEN_GHE")
          .notEmpty()
          .withMessage("Tên ghế không được để trống!")
          .isString()
          .withMessage("Tên ghế phải là chuỗi ký tự")
          .isLength({ max: 255 })
          .withMessage("Tên ghế không được vượt quá 255 ký tự!")
          .run(req),
        body("TOA_X")
          .notEmpty()
          .withMessage("Tọa độ X không được để trống!")
          .isInt()
          .withMessage("Tọa độ X phải là dạng số")
          .run(req),
        body("TOA_Y")
          .notEmpty()
          .withMessage("Tọa độ Y không được để trống!")
          .isInt()
          .withMessage("Tọa độ Y phải là dạng số")
          .run(req),
        body("TRANG_THAI")
          .notEmpty()
          .withMessage("Trạng thái không được để trống!")
          .isInt()
          .withMessage("Trạng thái phải là số nguyên")
          .run(req),
        body("ID_KHONG_GIAN")
          .notEmpty()
          .withMessage("ID không gian không được để trống!")
          .isInt()
          .withMessage("ID không gian phải là số nguyên")
          .custom(async (value, { req }) => {
      ...
      if (!updated) {
        return res
          ...
          .json({
            success: false,
            message: "Ghế không tồn tại hoặc dữ liệu không có thay đổi!",
          });
      }
      res
        .status(200)
        .json({ success: true, message: "Cập nhật thông tin ghế thành công!" });
    } catch (error) {
      res.status(500).json({ success: false, message: error.message });
    }
  }
