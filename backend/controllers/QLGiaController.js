        ...
        try {
            ...
            await Promise.all([
                query('ID_GIA')
                    ...
                    .isInt({ min: 1 }).withMessage('ID bảng giá phải là số nguyên dương!')
                    .custom(async (value) => {
                        const check = await giaModel.testid(value);
                        if (!check) throw new Error('Mã bảng giá này không tồn tại trên hệ thống!');
                        return true;
                    })
                    .run(req)
            ]);
            ...
            if (!item) {
                return res.status(404).json({ success: false, message: "Không tìm thấy thông tin bảng giá!" });
            }
...
static async  createGia (req, res)  {
        ...
        await Promise.all([
            body('TEN_GIA')
                ...
                body('MOTA')
                    ...
                    .run(req),
                body('DON_GIA')
                    .notEmpty().withMessage('Đơn giá không được bỏ trống!')
                    .isFloat({ min: 0 }).withMessage('Đơn giá phải là số và không được âm!')
                    .run(req),
                body('DANHMUC_GHE')
                    .notEmpty().withMessage('Danh mục ghế không được bỏ trống!')
                    .isInt({ min: 1 }).withMessage('Danh mục ghế phải là số nguyên dương!')
                    // .custom(async (value) => {
 ...
 static async updateGia(req, res) {
        try {
            ...
            await Promise.all([
                body('ID_GIA')
                    .notEmpty().withMessage('ID bảng giá cần cập nhật không được bỏ trống!')
                    .isInt({ min: 1 }).withMessage('ID bảng giá phải là số nguyên dương!')
                    .custom(async (value) => {
                        const check = await giaModel.testid(value);
                    ...
                    .run(req),
                body('TEN_GIA')
                    .notEmpty().withMessage('Tên bảng giá không được bỏ trống!')
                    .isLength({ max: 255 }).withMessage('Tên bảng giá tối đa 255 ký tự!')
                    .run(req),
                body('MOTA')
                    .optional({ nullable: true, checkFalsy: true })
                    .isLength({ max: 1000 }).withMessage('Mô tả tối đa 1000 ký tự!')
            ...
            if (!updateSuccess) {
                return res.status(500).json({
                    success: false,
                    message: 'Cập nhật bảng giá thất bại hoặc không có thay đổi mới nào được thực hiện!'
                });
            }
