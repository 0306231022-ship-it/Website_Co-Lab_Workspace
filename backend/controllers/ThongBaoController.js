  ...
  static async deleteThongBao(req, res) {
    try {
      ...
      await Promise.all([
        body("ID_THONGBAO")
          .notEmpty()
          .withMessage("ID thông báo cần xóa không được để trống!")
          .isInt({ min: 1 })
          .withMessage("ID thông báo phải là số nguyên dương!")
          .custom(async (value) => {
            const check = await thongBaoModel.testId(value);
            if (!check)
              throw new Error("ID thông báo này không tồn tại trên hệ thống!");
            return true;
          }),
      ]);
      ...
      if (!success) {
        return res
          .status(500)
          .json({ success: false, message: "Xóa thông báo thất bại!" });
      }
  ...
  static async deleteAllThongBaoByUserId(req, res) {
    try {
      ...
      return res
        ...
        .json({
          success: true,
          message: "Đã dọn sạch toàn bộ thông báo của người dùng này!",
        });
    } catch (error) {
      return res.status(500).json({ success: false, message: error.message });
    }
