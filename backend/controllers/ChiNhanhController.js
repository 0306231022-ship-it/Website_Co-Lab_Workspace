...
export default class ChiNhanhController {
  ...
  static async Them_ChiNhanh(req, res) {
    ...
    try {
      ...
      const them = await ChiNhanhModel.ThemChiNhanh(
        dulieu.TenCN,
        dulieu.DiaChi,
        DuongDan,
      );
  ...
  static async ChinhSua_Ten_DiaChi(req, res) {
    ...
    try {
        ...
        body("TenCN")
          .custom((value, { req }) => {
            ...
            if (!value?.trim() && !DiaChi?.trim()) {
              throw new Error(
                "Bạn phải nhập ít nhất tên chi nhánh hoặc địa chỉ chi nhánh!",
              );
            }
      ...
      const capnhat = await ChiNhanhModel.CapNhat_ThongTinMem(
        dulieu.TenCN,
        dulieu.DiaChi,
        dulieu.IDCN,
      );
  ...
  static async ChinhSua_TrangThai_ChiNhanh(req, res) {
    ...
    try {
      await Promise.all([
        body("ThoiGianApDung")
          ...
          .custom(async (value, { req }) => {
            ...
            if (startDate < now) {
              throw new Error(
                "Ngày chỉnh sửa chi nhánh phải lớn hơn hoặc bằng ngày hiện tại!",
              );
            }
            return true;
          })
      ...
      const update = await ChiNhanhModel.CapNhatTrangThai(
        parseInt(dulieu.IDCN),
        dulieu.ThoiGianApDung,
        parseInt(dulieu.TrangThai),
      );
      if (!update) {
        return res.status(500).json({
          success: false,
  ...
  static async TimKiem_ChiNhanh(req, res) {
    ...
    const offset = (page - 1) * limit;
    try {
      const danhsach = await ChiNhanhModel.TimKiem(
        limit,
        offset,
        DiaChi,
        TrangThai,
