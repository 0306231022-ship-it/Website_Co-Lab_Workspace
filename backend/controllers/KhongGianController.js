...
export default class KhongGianController {
  static async DanhSach_KhongGian(req, res) {
    ...
    try {
      await Promise.all([
        ...
        query("IDCN")
          .notEmpty()
          .withMessage("ID chi nhánh là thông tin bắt buộc")
          .isInt()
      ...
      const coTrangThai =
        trangthai !== undefined && trangthai !== null && trangthai !== "";
      const coTen = ten !== undefined && ten !== null && ten.trim() !== "";
      const danhsach =
        coTrangThai || coTen
          ? await KhongGianModel.TimKiem_KhongGian(
              ten,
              trangthai,
              ID,
              limit,
          ...
          : await KhongGianModel.LayDanhSach(limit, offset, ID);
      if (!danhsach) {
  ...
  static async Them_KhongGian(req, res) {
    ...
    try {
      await Promise.all([
        ...
        body("LoaiKG")
          .notEmpty()
          .withMessage("Loại không gian là thông tin bắt buộc")
          .isInt()
          .withMessage("Giá trị nhập vào phải là một số nguyên!")
          .isIn([0, 1, "0", "1"])
  ...
  static async ChinhSua_TenKhongGian(req, res) {
    ...
    try {
      await Promise.all([
        body("IDKG")
          .notEmpty()
          .withMessage("ID không gian là thông tin bắt buộc")
          .isInt()
          .withMessage("Giá trị nhập vào phải là một số nguyên!")
          .custom(async (value, { req }) => {
        ...
        body("TenKG")
          .notEmpty()
          .withMessage("tên không gian là thông tin bắt buộc")
          .run(req),
      ]);
      const errors = validationResult(req);
      if (!errors.isEmpty()) {
  ...
  static async ChinhSua_TrangThai_KhongGian(req, res) {
    ...
    try {
      await Promise.all([
        body("NgayBatDau")
          ...
          .custom(async (value, { req }) => {
            ...
            if (startDate < now) {
              throw new Error(
                "Ngày chỉnh sửa không gian phải lớn hơn hoặc bằng ngày hiện tại!",
              );
            }
        ...
        body("NgayHoanThanh")
          ...
          .custom(async (value, { req }) => {
            ...
            if (endDate < startDate) {
              throw new Error(
                "Ngày hoàn thành không gian phải lớn hơn hoặc bằng ngày bắt đầu!",
              );
            }
            return true;
          })
      ...
      const update = await KhongGianModel.CapNhatTrangThai(
        dulieu.IDKG,
        dulieu.NgayBatDau,
        dulieu.NgayHoanThanh,
      );
      if (!update) {
        return res.status(500).json({
          success: false,
  ...
  static async ChiTiet_KhongGian(req, res) {
    ...
    try {
      ...
      const [ThongTin_ChiNhanh, thongTin_KhongGian, DanhSach_ThietBi] =
        await Promise.all([
  ...
  static async TimKiem_KhongGian(req, res) {
    ...
    try {
      ...
      const timkiem = await KhongGianModel.TimKiem_KhongGian(
        ten,
        trangthai,
        IDCN,
        3,
