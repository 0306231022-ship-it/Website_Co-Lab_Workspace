...
function ChiTietChiNhanh() {
    ...
    useEffect(() => {
        const fetchChiNhanh = async () => {
            try {
                const response = await api.CallAPI(undefined, { url: `/admin/laychitiet?IDCN=${id}`, PhuongThuc: 0 });
                if (response.success) {
    ...
    useEffect(() => {
        const fetchKhongGian = async () => {
            try {
                const response = await api.CallAPI(undefined, { url: `/admin/laydanhsachkhonggian?IDCN=${id}&page=${page}&limit=${limit}`, PhuongThuc: 2 });
                if (response.success) {
                    setChiTiet2(response);
                } else {
    ...
    const filteredSpaces = danhSachKhongGian.filter(space => {
        const matchesSearch = space.TEN_KHONG_GIAN?.toLowerCase().includes(searchFilter.toLowerCase());
        const matchesType = typeFilter === "all" || space.LOAI_KHONG_GIAN?.toString() === typeFilter;
                        ...
                        <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto shrink-0">
                            ...
                            <div className="relative w-full sm:w-64">
                                ...
                                <input 
                                    ...
                                    className="w-full bg-slate-50/80 border border-slate-200 text-slate-800 text-xs font-medium rounded-xl pl-9 pr-4 py-3 focus:outline-none focus:border-slate-400 placeholder-slate-400 transition-all" 
                                />
                            </div>
                        </div>
                    </div>
    ...
    height={300}
                                        ...
                                        {/* Nội dung thông tin bên dưới ảnh */}
                                        <div className="p-5 flex flex-col flex-1">
                                            ...
                                            {/* Chân Card: Chứa giá và Nút hành động */}
                                            <div className="flex items-center justify-between mt-auto pt-4 border-t border-slate-100 relative z-30">
                                                ...
                                                {/* Khu vực nút bấm đồng bộ cấu trúc rounded-xl, padding và font */}
                                                <div className="relative z-40">
                                                    {isBaoTri ? (
                                                        ...
                                                        <button 
                                                            className="w-full mt-5 bg-blue-50 hover:bg-blue-600 text-blue-700 hover:text-white border border-blue-100 font-bold py-2.5 px-2.5 rounded-xl text-sm transition-all duration-300 flex items-center justify-center gap-2"
                                                        >
                                                            Xem sơ đồ
                                                        </button>
                                                    ) : (
                    ...
                    <div className="flex flex-col sm:flex-row items-center justify-between mt-8 pt-6 border-t border-slate-100 gap-4">
                        <p className="text-xs text-slate-400 font-medium">
                            Hiển thị <span className="font-bold text-slate-700">{filteredSpaces.length}</span> trên tổng số <span className="font-bold text-slate-700">{tongSoKhongGian}</span> không gian
                        </p>
                        <nav className="flex items-center gap-1.5">
                            <button onClick={()=>{setPage(p=>p-1)}} disabled={page === 1} className="w-8 h-8 flex items-center justify-center rounded-xl border border-slate-200 text-slate-400 hover:bg-slate-50 transition">
                            ...
                            <button onClick={()=>{setPage(p=>p+1)}} disabled={page >= Math.ceil(tongSoKhongGian / limit)} className="w-8 h-8 flex items-center justify-center rounded-xl border border-slate-200 text-slate-600 hover:bg-slate-50 transition active:scale-95">
