...
export default function QuanLyThietBi() {
    ...
    const filteredData = useMemo(() => {
        ...
        return danhSachThietBi.filter(item => 
        ...
        <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 space-y-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div className="space-y-1">
                    ...
                    <h1 className="text-2xl font-black text-slate-900 tracking-tight flex items-center">
                        Kho trang thiết bị <span className="ml-3 px-2.5 py-0.5 bg-indigo-50 text-indigo-600 text-xs rounded-full font-bold">{pagination.totalItems} items</span>
                    </h1>
                </div>
                <div className="flex items-center space-x-3">
                    <button type="button" className="px-4 py-2 bg-white border border-gray-200 hover:bg-gray-50 text-gray-700 text-xs font-bold rounded-xl shadow-3xs transition-all cursor-pointer flex items-center">
                        <i className="fa-solid fa-cloud-arrow-down mr-2"></i> Xuất Excel
                    </button>
                    <button type="button" className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold rounded-xl shadow-xs transition-all cursor-pointer flex items-center">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm flex items-center space-x-4">
                    ...
                    <div>
                        <p className="text-[11px] font-bold text-gray-400 uppercase tracking-wider">Tổng số lượng</p>
                        <p className="text-2xl font-black text-gray-900 mt-0.5 font-mono">{pagination.totalItems}</p>
                    </div>
                </div>
                <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm flex items-center space-x-4">
                    ...
                    <div>
                        <p className="text-[11px] font-bold text-gray-400 uppercase tracking-wider">Đang sử dụng</p>
                        <p className="text-2xl font-black text-gray-900 mt-0.5 font-mono">{pagination.totalItems > 0 ? Math.floor(pagination.totalItems * 0.9) : 0}</p>
                    </div>
                </div>
                <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm flex items-center space-x-4">
                    ...
                    <div>
                        <p className="text-[11px] font-bold text-gray-400 uppercase tracking-wider">Đang bảo trì/Sửa chữa</p>
                        <p className="text-2xl font-black text-gray-900 mt-0.5 font-mono">{pagination.totalItems > 0 ? Math.ceil(pagination.totalItems * 0.1) : 0}</p>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                                ...
                                <tr>
                                    <td colSpan={3} className="p-8 text-center text-slate-400">
                                        <div className="flex flex-col items-center justify-center space-y-2">
                                            ...
                                            <span className="text-xs font-semibold">Không tìm thấy thiết bị nào phù hợp!</span>
                                        </div>
                                    </td>
                                </tr>
                            ) : (
                                filteredData.map((item) => (
                                    <tr key={item.ID_THIET_BI} className="hover:bg-slate-50/50 transition-colors">
                                        <td className="p-4 pl-6">
                                            <div className="flex items-center space-x-3">
                                                ...
                                                <div>
                                                    <p className="font-black text-gray-900">{item.TEN_THIET_BI}</p>
                                                    <p className="text-[11px] text-gray-400 font-semibold mt-0.5">ID hệ thống: #{item.ID_THIET_BI}</p>
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td className="p-4 pr-6 text-right">
                                            <button 
                                                type="button" 
                                                className="w-8 h-8 rounded-lg text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 transition cursor-pointer" 
                                                title="Xem chi tiết"
                                            >
                ...
                <div className="px-6 py-4 border-t border-gray-100 bg-slate-50/50 flex flex-col sm:flex-row items-center justify-between gap-4">
                    <p className="text-xs text-gray-500 font-medium">
                        Hiển thị <span className="font-bold text-gray-900">{itemStart}</span> đến <span className="font-bold text-gray-900">{itemEnd}</span> trong số <span className="font-bold text-gray-900">{pagination.totalItems}</span> thiết bị
                    </p>
