    ...
    const fetchChiTietGia = async () => {
      ...
      try {
        ...
        if (
          targetData &&
          targetData !== null &&
          (targetData.ID_GIA !== undefined || targetData.TEN_GIA !== undefined)
        ) {
          ...
          ThongBao.ThongBao_Loi(
            res?.message ||
              res?.thongBao ||
              "Không thể tải hoặc sai cấu trúc thông tin đơn giá!",
          );
          setGia(null);
        }
      } catch (error) {
    ...
    return (
      <div className="w-full bg-white border border-slate-200 rounded-2xl p-8 text-center space-y-4">
        ...
        <h3 className="text-lg font-bold text-slate-700">
          Không tìm thấy dữ liệu đơn giá #{id}
        </h3>
        <button
          type="button"
          onClick={() => router.back()}
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap items-center gap-2 text-xs font-semibold text-gray-400">
        ...
        >
          <i className="fa-solid fa-arrow-left mr-1.5 text-[10px] text-slate-400 group-hover:-translate-x-0.5 transition-transform"></i>
          Quay lại
        </button>
        <Link
          href="/danh-muc"
        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400 font-bold uppercase tracking-wider mt-0.5">
              ...
              <span className="ml-2 font-mono text-slate-500 bg-slate-100 px-1.5 py-0.5 rounded">
                DM_GHE: {gia.DANHMUC_GHE}
              </span>
            </p>
          </div>
        </div>
          ...
          <div className="bg-white border border-slate-200/80 rounded-2xl shadow-xs overflow-hidden divide-y divide-slate-100">
            <div className="p-6 sm:p-8 space-y-4">
              ...
              <div className="text-4xl font-black text-slate-900 font-mono tracking-tight flex items-baseline gap-2">
                ...
                <span className="text-sm font-sans text-slate-400 font-semibold tracking-normal">
                  / Giờ sử dụng
                </span>
              </div>
            </div>
            ...
            <div className="p-6 sm:p-8 space-y-3 bg-slate-50/40">
              <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest flex items-center gap-1.5">
                <i className="fa-solid fa-align-left text-slate-400"></i> Mô tả
                chi tiết quy định áp dụng
              </h3>
              <p className="text-sm text-slate-600 leading-relaxed font-medium">
                {gia.MOTA ? (
                  ...
                  <span className="italic text-slate-400">
                    Chưa có thông tin mô tả chi tiết cho đơn giá này.
                  </span>
                )}
              </p>
            </div>
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-white border border-slate-200/80 rounded-2xl shadow-xs p-6 space-y-5">
            <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest border-b border-slate-100 pb-3 flex items-center gap-1.5">
              <i className="fa-solid fa-history text-slate-400"></i> Nhật ký &
              Trạng thái
            </h3>
