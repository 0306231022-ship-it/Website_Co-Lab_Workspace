    ...
    <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <h1 className="text-2xl font-black text-slate-900 tracking-tight">
            Cấu hình Giá áp dụng
          </h1>
          <p className="text-xs text-gray-400 font-medium mt-0.5">
            Thiết lập các khung giá tiêu chuẩn theo giờ, ngày, tháng để áp dụng
        ...
        <div>
          <button
            onClick={() => {
              OpenMoDal(undefined, { TenTrang: "ThenGia" });
            }}
      ...
      <div className="bg-white border border-gray-100 rounded-2xl shadow-xs overflow-hidden">
              ...
              {!loading && filteredBangGia.length === 0 ? (
                ...
                filteredBangGia.map((item) => {
                  ...
                  return (
                    <tr
                      key={item.ID_GIA}
                      className={`hover:bg-gray-50/50 transition-colors ${!isActive ? "bg-red-50/5" : ""}`}
                    >
                      <td className="py-4 px-6">
                        <span
                          className={`font-black block text-sm ${isActive ? "text-gray-900" : "text-gray-400 line-through"}`}
                        >
                          {item.TEN_GIA}
                        </span>
                      ...
                      <td className="py-4 px-6 text-right">
                        <span
                          className={`font-black font-mono text-sm ${isActive ? "text-gray-900" : "text-gray-400 line-through"}`}
                        >
                      ...
                      <td className="py-4 px-6 text-center">
                        <button
                          onClick={() =>
                            router.push(
                              `/admin/QuanLiGia/chitietgia/${item.ID_GIA}`,
                            )
                          }
        ...
        <div className="px-6 py-4 bg-gray-50 border-t border-gray-100 flex flex-col sm:flex-row items-center justify-between text-[11px] font-medium text-gray-500 gap-3">
          <span>
            Hiển thị {filteredBangGia.length} của tổng số{" "}
            {pagination?.totalItems || 0} cấu hình bảng giá áp dụng
          </span>
