...
export default function DanhMucGhe() {
  ...
  const filteredData = useMemo(() => {
    return danhSachs.filter((item) => {
      ...
      const matchStatus =
        filterStatus === "" || item.TRANG_THAI.toString() === filterStatus;
  ...
  return (
    <div className="p-4 sm:p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-white p-5 rounded-2xl border border-slate-100 shadow-3xs">
        <div className="flex items-center gap-3.5">
          ...
          <div>
            ...
            <p className="text-xs font-medium text-slate-400 mt-0.5">
              Cấu hình phân loại chỗ ngồi và thiết lập đơn giá tuyến tính theo
              giờ
            </p>
          </div>
        </div>
        <button
          onClick={() => {
            OpenMoDal(undefined, { TenTrang: "ThemDanhMucGhe" });
          }}
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-xs overflow-hidden flex flex-col">
            ...
            <tbody className="divide-y divide-slate-50 text-xs font-medium text-slate-700">
              {isLoading ? (
                ...
                filteredData.map((item) => (
                  <tr
                    key={item.ID_DANHMUC}
                    className={`hover:bg-slate-50/50 transition-colors ${item.TRANG_THAI !== 1 ? "bg-slate-50/30" : ""}`}
                  >
                    <td className="px-6 py-4 font-mono font-bold text-slate-400 text-center bg-slate-50/10">
                      {item.ID_DANHMUC}
                    </td>
                    <td
                      className={`px-6 py-4 ${item.TRANG_THAI !== 1 ? "opacity-70" : ""}`}
                    >
                      <div
                        className={`font-bold text-slate-900 ${item.TRANG_THAI !== 1 ? "line-through" : ""}`}
                      >
                        {item.TEN_DANHMUC}
                      </div>
                      {item.TRANG_THAI !== 1 && (
                        <span className="text-[10px] text-amber-600 font-medium block mt-0.5">
                          <i className="fa-solid fa-screwdriver-wrench text-[9px] mr-1"></i>{" "}
                          Đang tạm đóng / bảo trì
                        </span>
                      )}
                    </td>
                    <td
        ...
        <div className="px-6 py-3.5 border-t border-slate-100 bg-slate-50/30 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs font-semibold text-slate-500">
          ...
          <div className="flex items-center gap-1">
            ...
            <button
              type="button"
              className="px-3 py-1.5 bg-emerald-600 border border-emerald-600 text-white rounded-lg font-bold"
            >
              {page}
            </button>
