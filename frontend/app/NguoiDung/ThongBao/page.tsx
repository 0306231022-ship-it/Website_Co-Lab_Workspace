...
function ThongBaoComponent() {
  ...
  const handleXoaMot = async (id: number) => {
    const XacNhan = await Thongbao.ThongBao_XacNhanTT(
      "Bạn có chắc muốn xóa dòng thông báo này?",
    );
    if (!XacNhan) return;
    try {
      const formData = new FormData();
      formData.append("ID_THONGBAO", String(id));
      const xoa = await api.CallAPI(formData, {
        url: "/NguoiDung/XoaTheoid",
        PhuongThuc: 1,
      });
      if (xoa.success) {
  ...
  const handleXoaTatCa = async () => {
    ...
    if (!XacNhan) return;
    try {
      const xoa = await api.CallAPI(undefined, {
        url: "/NguoiDung/XoaTatCa",
        PhuongThuc: 1,
      });
      if (xoa.success) {
        setThongBao([]);
    ...
    <div className="p-4 mx-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Cập nhật những thông tin mới nhất về lịch đặt và hệ thống.
          </p>
        </div>
          ...
          thongBao.map((item) => {
            ...
            return (
              ...
              >
                ...
                {/* Icon theo Loại */}
                <div
                  className={`w-12 h-12 rounded-full flex items-center justify-center shadow-sm shrink-0 text-xl border ${config.bgIcon}`}
                >
                ...
                <div className="flex-1 pr-10">
                  <div className="flex items-center gap-2 mb-1">
                    <span
                      className={`text-xs font-bold uppercase tracking-wider ${config.badgeColor}`}
                    >
                      {config.label}
                    </span>
                  ...
                  <h3
                    className={`text-base flex items-center gap-2 ${isChuaDoc ? "font-bold text-slate-900" : "font-semibold text-slate-600"}`}
                  >
                    {item.TIEU_DE}
                  ...
                  <p
                    className={`text-sm mt-1 leading-relaxed ${isChuaDoc ? "text-slate-600" : "text-slate-400"}`}
                  >
                    {item.NOI_DUNG}
                  </p>
                </div>
      ...
      {hasMore && thongBao.length > 0 && (
        <div className="pt-6 text-center">
          ...
          >
            <i
              className={`fa-solid ${isLoading ? "fa-spinner animate-spin" : "fa-spinner"}`}
            ></i>
