...
function ChinhSuaKhongGian() {
    ...
    // --- Lấy thông tin ban đầu từ Server ---
    useEffect(() => {
    ...
    const handleLuuCauHinh = async () => {
        ...
        try {
            ...
            if(anhChiNhanh!==null){
                 ...
                 if (!resNhom3.success) {
                    ThongBao.ThongBao_Loi("Lỗi cập nhật thông tin: " + resNhom3.message);
                    setloading(false);
                    return;
                }
          ...
          <ul className="list-disc list-inside text-xs text-red-600 font-semibold space-y-1 pl-1">
            {errors.map((error, index) => (
              <li key={index}>{error}</li>
                        ...
                        {/* 4. KHU VỰC: CẢNH BÁO LỊCH ĐẶT & ẤN ĐỊNH KHUNG GIỜ BẢO TRÌ */}
                        <div id="emptyTimeAlert" className="bg-amber-50 border border-amber-200 rounded-2xl p-4 space-y-3.5 animate-in fade-in duration-300">
                            ...
                            {/* Cấu hình khung giờ bảo trì trực quan */}
                    ...
                    <div className="px-6 py-4 bg-slate-50 border-t border-gray-100 flex items-center justify-end space-x-2">
                        <button type="button"  className="px-4 py-2 bg-white hover:bg-gray-100 text-gray-500 text-xs font-bold rounded-xl border border-gray-200 transition-all cursor-pointer">
                            Hủy bỏ
                        </button>
                        <button 
                            type="button"
