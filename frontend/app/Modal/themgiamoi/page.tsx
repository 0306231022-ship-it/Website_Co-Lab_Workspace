...
export default function ThemGiaMoi() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // --- FIX LỖI FORMDATA Ở ĐÂY ---
        // Chuyển đổi Object State thành đối tượng FormData thực thụ
        const submitData = new FormData();
        submitData.append("TEN_GIA", formData.TEN_GIA.trim());
        submitData.append("MOTA", formData.MOTA || "");
        submitData.append("DON_GIA", String(formData.DON_GIA)); // FormData bắt buộc gửi dạng string hoặc Blob
        ...
        try {
            ...
            const res = await api.CallAPI(submitData, { url: "/admin/themgiamoi", PhuongThuc: 1 });
            ...
            if (res && res.success) {
                // SỬA: Đổi từ ThongBao_Loi sang hàm thông báo thành công (Nếu file của bạn dùng tên khác hãy đổi lại cho khớp nhé)
                if (ThongBao.ThongBao_ThanhCong) {
                    ThongBao.ThongBao_ThanhCong(res.message || "Thêm bảng giá mới thành công!");
                } else {
                ...
                ThongBao.ThongBao_Loi(res?.message || "Thêm bảng giá thất bại. Vui lòng thử lại!");
            }
        } catch (error) {
    ...
    return (
        <div className="bg-white rounded-2xl border border-slate-100 shadow-2xl w-full max-w-md overflow-hidden relative z-10 flex flex-col max-h-[90vh]">
            ...
            <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                <div className="flex items-center space-x-2.5">
                    ...
                    <div>
                        <h3 className="text-sm font-bold text-slate-900">Thêm đơn giá cho danh mục</h3>
                        <p className="text-[11px] text-slate-400 font-medium">Thiết lập mức giá mới gắn liền với loại ghế</p>
                    </div>
                </div>
                <button 
                    type="button" 
            ...
            {/* FORM BODY */}
            <form id="add-rate-form" onSubmit={handleSubmit} className="flex flex-col flex-1 overflow-y-auto">
                <div className="p-6 space-y-4 flex-1">
                    
                    {/* BỔ SUNG KHUNG HIỂN THỊ LỖI VALIDATE TỪ BACKEND (NẾU CÓ) */}
                    {err && err.length > 0 && (
                    ...
                    {/* ID HỆ THỐNG (READ-ONLY) */}
                    <div className="space-y-1.5">
                        <label className="text-xs font-bold text-slate-500 uppercase tracking-wide flex items-center gap-1">
                            Mã đơn giá mới (ID hệ thống)
                        </label>
                        <div className="relative flex items-center">
                            ...
                            />
                            <span className="absolute right-3.5 text-[10px] bg-slate-200 text-slate-600 font-bold px-1.5 py-0.5 rounded font-sans tracking-normal flex items-center gap-1">
                                <i className="fa-solid fa-lock text-[9px]"></i> Auto-increment
                            </span>
                        </div>
                    </div>
                    ...
                    <div className="space-y-1.5">
                        <label htmlFor="DON_GIA" className="text-xs font-bold text-slate-600 uppercase tracking-wide flex items-center gap-1">
                            Đơn giá tiêu chuẩn (VND / 1 Giờ) <span className="text-rose-500">*</span>
                        </label>
                        <div className="relative flex items-center">
                            <input 
                                type="number" 
                            ...
                            />
                            <span className="absolute right-3.5 text-xs font-bold text-slate-400 pointer-events-none">đ / giờ</span>
                        </div>
                    </div>

                    {/* TRẠNG THÁI ÁP DỤNG */}
                    <div className="space-y-1.5">
                        <label htmlFor="trang-thai" className="text-xs font-bold text-slate-600 uppercase tracking-wide flex items-center gap-1">
                            Trạng thái áp dụng liền?
                        </label>
                        <select id="trang-thai" className="w-full text-sm px-3.5 py-2.5 bg-white border border-slate-200 rounded-xl shadow-3xs focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 transition-all font-medium text-slate-700">
                            <option value="1">Kích hoạt ngay (Hệ thống tự gỡ bỏ giá cũ)</option>
                        ...
                        <span className="text-[10px] text-amber-600 font-medium block leading-normal">
                            <i className="fa-solid fa-triangle-exclamation mr-0.5"></i> Khi chọn &quot;Kích hoạt ngay&quot;, hệ thống tự động tắt (Inactivate) các đơn giá cũ của danh mục ghế này.
                        </span>
                    </div>

                    {/* MÔ TẢ */}
                    <div className="space-y-1.5">
                        <label htmlFor="MOTA" className="text-xs font-bold text-slate-600 uppercase tracking-wide">Mô tả / Lý do điều chỉnh giá</label>
                        <textarea 
                            id="MOTA" 
                            name="MOTA"
                            rows={2} 
                ...
                <div className="px-6 py-3.5 border-t border-slate-100 bg-slate-50/50 flex items-center justify-end space-x-2 shrink-0">
                    ...
                    >
                        {loading ? (
                            ...
                            <>
                                <i className="fa-solid fa-cloud-arrow-up mr-1.5"></i> Cập nhật giá mới
                            </>
                        )}
                    </button>
                </div>
