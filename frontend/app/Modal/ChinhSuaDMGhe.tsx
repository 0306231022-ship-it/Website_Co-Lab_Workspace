...
// Định nghĩa Props chuẩn theo hệ thống Modal
interface ChildComponentProps {
    DuLieu?: any; // Nhận nguyên Object chiếc ghế từ OpenMoDal gửi sang
    url?: string | null;
    onClose?: () => void;
}
...
export default function Chinhsuadmghe({ DuLieu, onClose }: ChildComponentProps) {
    ...
    useEffect(() => {
        if (!DuLieu) {
            ...
            onClose?.(); // Không có dữ liệu thì tự động đóng Modal
            return;
        }
        ...
        setFormData({
            ...
            TRANG_THAI: DuLieu.TRANG_THAI !== undefined 
                ? Number(DuLieu.TRANG_THAI) 
                : (DuLieu.trang_thai !== undefined ? Number(DuLieu.trang_thai) : 1)
        });
    }, [DuLieu]);
            ...
            {err && err.length > 0 && (
                <div className="p-3 bg-rose-50 border border-rose-200 rounded-xl space-y-1">
                    ...
                    <ul className="list-disc list-inside text-xs text-rose-600 font-medium space-y-0.5">
                        {err.map((errorMsg, index) => <li key={index}>{errorMsg}</li>)}
                    </ul>
                </div>
            )}
